...
#include <string.h>
#include <stdbool.h>
const int MaxPasswordSize = 8;
bool check(const char* salt, const char* pass, const char* text){
    if(strcmp(crypt(salt,text),pass))
    return true;
    return false;
}
bool brute(const char* salt, const char* pass){
    char arr_chars[] = {"0123456789AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz !\"#$%&\'()*+,-./:;<=>?@[\\]^_`{|}~"};
    int len = strlen(arr_chars);
    char cur_word[8];
    int pos[8];
    ...
    for(int i=1; i<=MaxPasswordSize; i++){
        ...
        for(int j=0; j<i; j++){
            pos[j] = 0;
            cur_word[j] = arr_chars[pos[j]]; 
        }
        cur_word[i] = '\0';
    while(!loopcompleted){
          if (check(salt, pass, cur_word))
                return true;
        
            // One iteration of brute-force
            bool shift = true;
            
            for (int cur_char = 0; cur_char <= i; cur_char++)
                // If shift - change character
                if (shift)
                {
                    // If all character are done
                    if (pos[cur_char] == len - 1)
                    {
...
bool dict(const char* salt, const char* pass){
    FILE*f;
    char cur_word[128];
    ...
    while(fgets(cur_word, 128, f)){
        if(check(salt, pass, cur_word))
        {
            printf("Password was cracked succesfully\nThe password is : ");
            printf("%s\n", cur_word);
            return true;
   }
    }
...
{
    if(argc!=2){
        printf("USAGE: ./crack encryptedpassword");
        printf("\n");
        return 1;
    }
    char salt[2];
    strncpy(salt,argv[1],2);
    printf("Attacking using inbuilt dictionary\n");
    if(dict(argv[1],salt))
    return 0;

     printf("Dictionary attack failed\n");
    printf("Attacking using brute force\n");
    
    if(brute(salt,argv[1]))
    return 0;
    printf("Brute force attack gave no results password could not be cracked :( \n");
