...
{
        // creating a copy of line
        char copyofline[strlen(line) + 1];
        strcpy(copyofline, line);
        copyofline[strlen(line)] = '\0';
        
        char *methodcopy, *requesttargetcopy, *httpversioncopy;
        
        // filtering out tokens of method, requesttarget, http-version from copyofline
        methodcopy = strtok(copyofline, " ");
        ...
        // add null terminators at end of all three
        ...
        // filter out tokens of abs_path and query from request target
        ...
        // if abs_path contains '?' or query contains '"' then throw error 400
        if (strchr(abs_path, '?') != NULL || strchr(query, '"') != NULL)
        {
            error(400);
        }
    return true;
